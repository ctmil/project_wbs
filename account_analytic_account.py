...
class AccountAnalyticAccount(base_stage, orm.Model):
    ...
    def _child_count(self, cr, uid, ids, account_class, arg, context=None):
        ...
        for analytic_account in self.browse(cr, uid, ids, context=context):
            deliverable_ids = self.pool.get(
                ...
                cr, uid, [('parent_id', '=', analytic_account.id),
                          ('account_class', '=', account_class)], context=ctx)
            if deliverable_ids:
    ...
    def _resolve_analytic_account_id_from_context(self, cr, uid, context=None):
        ...
        'default_parent_id' context key, or None if it cannot be resolved
        to a single account.analytic.account
        """
        if context is None:
            context = {}
    ...
    def _get_project_account_id(self, cr, uid, ids, prop, unknow_none,
                                unknow_dict):
        if not ids:
            return []
	...
	'child_ids': fields.one2many('account.analytic.account','parent_id','Child accounts'),
        ...
        'type': fields.selection([('view','Analytic View'), ('normal','Analytic Account'),('contract','Contract or Project'),('template','Template of Contract')], 'Type of Account', required=False,
    ...
    def name_search(self, cr, uid, name, args=None, operator='ilike',
        ...
        args = args[:]
        accountbycode = self.search(cr, uid,
                                    [('complete_wbs_code', 'ilike',
                                      '%%%s%%' % name)]+args, limit=limit,
                                    context=context)
        accountbyname = self.search(cr, uid, [('complete_wbs_name',
                                               'ilike', '%%%s%%' % name)]+args,
